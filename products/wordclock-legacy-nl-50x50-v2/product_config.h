#pragma once

#define PRODUCT_ID "wordclock-legacy-nl-50x50-v2"
#define FIRMWARE_VERSION "legacy-nl-50x50-v2-26.2.6-rc.12"
#define UI_VERSION "ui-legacy-nl-50x50-v2-26.2.6-rc.12"
#define PRODUCT_VARIANT_LEGACY 1
#define SETUP_ASSUME_DONE_IF_LEGACY_CONFIG 1
#define DATA_PIN 4
#define SUPPORT_OTA_V2 1
#define SETUP_BLINK_ENABLED 0
#define SETUP_BLINK_LED_COUNT 0

// Log level: LOG_LEVEL_DEBUG, LOG_LEVEL_INFO, LOG_LEVEL_WARN, LOG_LEVEL_ERROR
// #define DEFAULT_LOG_LEVEL LOG_LEVEL_ERROR

// Update channel: "stable", "early", "develop"
// #define DEFAULT_UPDATE_CHANNEL "stable"
