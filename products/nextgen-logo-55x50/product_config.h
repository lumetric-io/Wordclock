#pragma once

#define PRODUCT_ID "nextgen-logo-55x50"
#define FIRMWARE_VERSION "nextgen-logo-55x50-26.08.19-photo.1"
#define UI_VERSION "ui-nextgen-logo-55x50-26.08.19-photo.1"
#define PRODUCT_VARIANT_LOGO 1
#define DATA_PIN 4
#define LOGO_DATA_PIN 18
#define SUPPORT_OTA_V2 1
#define BLE_PROVISIONING_ENABLED 0
#define WIFI_MANAGER_ENABLED 1
#define LED_STATUS_EVENTS_ENABLED 0
