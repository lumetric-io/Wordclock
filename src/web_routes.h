...
void setupWebRoutes() {
  ...
  // Grid is compile-time only (one variant per product); the runtime
  // /getGridVariant, /listGridVariants, /setGridVariant endpoints have
  // been removed along with the dashboard's grid selector.
  ...
  // Wi-Fi enrolment from the admin page. Counterpart to connectFactoryWifi():
  // a chip that came up on the workshop network is reachable here, so the
  // operator can hand it its real network without ever opening the portal.
  //
  ...
  // Sell mode endpoints (force SELL_MODE_HOUR:SELL_MODE_MINUTE display)
